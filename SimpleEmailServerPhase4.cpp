...
using namespace std;
//Get numfds
int getNumfds(vector<int> activeSockets,int sockfdListen){
	int maximum=sockfdListen;
	for(int i:activeSockets){
		if(i>maximum){
			maximum=i;
		}
	}
...
int main(int argc, char* argv[]){
	...
	fd_set readfdsG;
	FD_ZERO(&readfdsG);
	FD_SET(sockfdListen,&readfdsG);
	//Create all vectors
	vector<int> activeSockets;
	vector<string> activeunames;
	vector<FILE*> activeFiles;
	vector<uint32_t> activeFileSizes;
	vector<bool> authDone;
	vector<bool> isTransferringFile;
	vector<bool> waitingForCommand;
	...
	while(true){
		...
		if(retval){
			bool listenSet=FD_ISSET(sockfdListen,&readfdsL);
			vector<int> readySocketIndices;
			for(int i=0;i<activeSockets.size();i++){
				if(FD_ISSET(activeSockets[i],&readfdsL)){
					readySocketIndices.push_back(i);
				}
			}
			if(listenSet){
				//Accept
				...
				int retval=accept(sockfdListen,(struct sockaddr*)&caddr,
					&addrlen);
				if(retval==-1){
					cerr<<
					"AcceptFailed"<<endl;
					...
					int sockfd=retval;
					activeSockets.push_back(sockfd);
					activeunames.push_back("");
					activeFiles.push_back(NULL);
					activeFileSizes.push_back(0);
					authDone.push_back(false);
					isTransferringFile.push_back(false);
					waitingForCommand.push_back(false);
					wishToCloseConn.push_back(false);
					FD_SET(sockfd,&readfdsG);
				}
			}
			for(int idx:readySocketIndices){
				...
				if(waitingForCommand[idx]){
					...
					bool changed=recvString(message,remMessage,maxLen,sockfd);
					delete[] remMessage;
					//Support LIST at end if time permits
					if(!changed){
						wishToCloseConn[idx]=true;
						delete[] message;
					}else{
